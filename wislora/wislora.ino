...
void processIncomingMessage(char *message) {
    ...
    int TVOC, eCO2, H2, Ethanol, light, brightness;

    int matched = sscanf(jsonBuffer,
                         "{\"T\": \"%9[^\"]\", \"humidity\": \"%9[^\"]\", \"TVOC\": %d, \"eCO2\": %d, \"H2\": %d, \"Ethanol\": %d, \"light\": %d, \"brightness\": %d}",
                         tempStr, humidityStr, &TVOC, &eCO2, &H2, &Ethanol, &light, &brightness);
    ...
    //  Prepare LoRa payload (8 bytes)
    uint8_t lora_payload[11] = {
        (uint8_t) (temp * 10),   // Scale temp to preserve decimal
        (uint8_t) (humidity * 10), // Scale humidity
        (uint8_t) TVOC, 
        (uint8_t)(eCO2 >> 8), (uint8_t)(eCO2 & 0xFF),
        (uint8_t)(H2 >> 8), (uint8_t)(H2 & 0xFF),
        (uint8_t)(Ethanol >> 8), (uint8_t)(Ethanol & 0xFF),
        (uint8_t) brightness,
        (uint8_t) light         // Light as integer
    };
